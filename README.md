...
### 설정 다이얼로그
- 시작 시 자동으로 설정 창 표시
- **P** 키로 런타임 중 설정 재변경 가능
- 변경 즉시 적용 (노출, UDP 주소, 해상도)
...
| 전송 조건 | 호모그래피 설정 완료 + S 키 ON + 영역 내 포인트 존재 |
...
IRViewer에서 **S 키**로 전송을 시작하면 실시간으로 좌표를 시각화합니다.
...
### UDP 좌표가 전송되지 않음
1. **S 키**로 전송 ON 상태 확인 (OSD 좌상단 표시)
